...
class Schedule:
    def __init__(self, start_date, end_date) -> None:
        try:
            self.start_date = datetime.datetime.strptime(start_date, '%Y-%m-%d').replace(hour=0, minute=0, second=0)
            self.end_date = datetime.datetime.strptime(end_date, '%Y-%m-%d').replace(hour=23, minute=59, second=59)
    ...
    def next_weekday(self, days) -> datetime.datetime:  # A function  to get the next weekday from a given date
        min_date = 7
        for day in days:
        ...
        for index, row in self.df.iterrows():
            ...
            if len(row['Room']) - 1: # check if it has multiple rooms => create multiple events
                ...
                for i, room in enumerate(row['Room']):
                    e = Event()
                    start_time = datetime.datetime.strptime(row['Time'].split()[0 + 3*i], '%H:%M') 
                    start_time = datetime.datetime.combine(self.next_weekday(row['Days'].split(" ")[i]), start_time.time())
                    end_time = datetime.datetime.strptime(row['Time'].split()[2 + 3*i], '%H:%M') 
                    end_time = datetime.datetime.combine(self.next_weekday(row['Days'].split(" ")[i]), end_time.time())
                    e.add('summary', row['Course Title'])
                    e.add('dtstart', start_time)
                    e.add('dtend', end_time)
                    e.add('rrule', {'freq': 'weekly', 'until': self.end_date, 'byday': rrdays[row['Days'].split(' ')[i]]})
                    e.add('location', room)
                ...
                start_time = datetime.datetime.combine(self.next_weekday(row['Days'].split(", ")), start_time.time())
                end_time = datetime.datetime.strptime(row['Time'].split()[2], '%H:%M') 
                end_time = datetime.datetime.combine(self.next_weekday(row['Days'].split(", ")), end_time.time())
                e.add('summary', row['Course Title'])
                e.add('dtstart', start_time)
                e.add('dtend', end_time)
                days =[rrdays[x] for x in row['Days'].split(', ')]
                e.add('rrule', {'freq': 'weekly', 'until': self.end_date, 'byday': days})
